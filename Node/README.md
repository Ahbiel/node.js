...
O Postman é uma plataforma de desenvolvimento de API (Application Programming Interface) que oferece ferramentas para simplificar o processo de criação, teste e documentação de APIs. É uma ferramenta muito popular entre desenvolvedores de software, engenheiros de software e equipes de desenvolvimento de API. Algumas das principais funcionalidades e recursos do Postman incluem:

O Postman permite que você crie facilmente solicitações HTTP, como GET, POST, PUT, DELETE e outros, para interagir com APIs e serviços da web.

Para testar, vamos adiciar uma nova requisição dentro do postman e então colocar a rota como **http://localhost:3000/** que foi a rota que criamos no arquivo **index.js** 

Podemos também enviar dados pelo POST, selecionando o campo **Body** dentro do postman, e depois **raw/JSON**, buscando os dados enviados através do req.body
```js
app.post('/create',(req,res)=>{
    const {name, price} = req.body
    console.log(name,price)
    res.json({message: `O produto ${name} com o preço ${price} foi criado com sucesso`})
}) //podemos mandar uma requisição do tipo post para a rota http://localhost:3000/create
```
